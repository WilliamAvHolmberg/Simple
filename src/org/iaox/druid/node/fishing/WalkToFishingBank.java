...
public class WalkToFishingBank extends Node{
	...
	public boolean active() {
		return !skillingProvider.shouldPeformSkillingAction() && !fishingProvider.inBankArea() ;
	}
	...
	public void run() {
		//initialize a boolean called exception
				...
				travelExceptions = combatProvider.getAssignment().getTravelExceptionsToBank();
				if(travelExceptions != null){
					...
					for (TravelException travelException : travelExceptions) {
						...
						//if the area does not contain our player, go to next travelexception
						if(travelException.getArea().contains(methodProvider.myPlayer())){
							...
							case TELEPORT:
								...
								//shall set exception to false if inventory does not contain teleport
